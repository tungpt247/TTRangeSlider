...
@property (nonatomic) CALayer *midLine;
...
- (void)initialiseControl {
	...
	self.midLine = [CALayer layer];
	self.midLine.backgroundColor = self.tintColor.CGColor;
	[self.layer addSublayer:self.midLine];
...
- (void)layoutSubviews {
	...
	self.sliderLine.frame = CGRectMake(lineLeftSide.x, lineLeftSide.y, lineRightSide.x-lineLeftSide.x, 4);
	self.sliderLine.backgroundColor = self.lineColor.CGColor;
	if (!self.didInit) {
		CGFloat xpos = self.leftHandle.frame.origin.x + HANDLE_DIAMETER;
		CGFloat xposRight = self.rightHandle.frame.origin.x + HANDLE_DIAMETER;
		CGFloat width = xposRight - xpos - HANDLE_DIAMETER;
		self.midLine.frame = CGRectMake(self.leftHandle.frame.origin.x + HANDLE_DIAMETER, lineLeftSide.y, width, 4);
		self.didInit = true;
	}
...
- (void)updateHandlePositions {
	...
	CGFloat xposLeft = [self getXPositionAlongLineForValue:self.selectedMinimum];
	CGFloat xposRight = [self getXPositionAlongLineForValue:self.selectedMaximum];

	self.midLine.frame = CGRectMake(xposLeft, CGRectGetMinY(self.midLine.frame), xposRight - xposLeft, 4);

	CGPoint leftHandleCenter = CGPointMake(xposLeft, CGRectGetMidY(self.sliderLine.frame));
	self.leftHandle.position = leftHandleCenter;
	
	CGPoint rightHandleCenter = CGPointMake(xposRight, CGRectGetMidY(self.sliderLine.frame));
